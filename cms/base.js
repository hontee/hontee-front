...
CMS.addSubmitHandler = function(data, $obj) {
  ...
  if (r.success) {
    $.messager.alert(CMS.msg.info, CMS.msg.addS, "info");
    $obj.addWin.window('close'); // 关闭窗口
    CMS.reload($obj);
  } else {
...
CMS.editSubmitHandler = function(data, $obj) {
  var r = $.parseJSON(data);
  if (r.success) {
    $.messager.alert(CMS.msg.info, CMS.msg.editS, "info");
    $obj.editWin.window('close'); // 关闭窗口
    CMS.reload($obj);
  } else {
    $.messager.alert(CMS.msg.error, r.error.message, "error");
  }
    ...
    $.messager.confirm(CMS.msg.info, CMS.msg.delConfirm, function(ok){
      if (ok){
        ...
        $.post(url, function(data) {
          var r = $.parseJSON(data);
          if (r.success) {
            $.messager.alert(CMS.msg.info, CMS.msg.removeS, "info");
            CMS.reload($obj);
          } else {
...
CMS.batchDeleteSubmitHandler = function($obj, uri) {
  ...
  if (rows.length > 0) {
	...
	$.messager.confirm(CMS.msg.info, CMS.msg.delConfirm, function(ok){
      if (ok){
        var url = '/cms/' + uri + '/delete/batch?ids=' + ids;
        $.post(url, function(data) {
          var r = $.parseJSON(data);
          if (r.success) {
            $.messager.alert(CMS.msg.info, CMS.msg.cutS, "info");
            CMS.reload($obj);
          } else {
            $.messager.alert(CMS.msg.removeF, r.error.message, "error");
          }
